...
class Recognizer(AudioSource):
	...
	def recognize_houndify(self, audio_data, client_id, client_key, show_all=False):
		...
		if "Disambiguation" not in result or result["Disambiguation"] is None:
			raise UnknownValueError()
		return result['Disambiguation']['ChoiceData'][0]['Transcription'], result['Disambiguation']['ChoiceData'][0][
			'ConfidenceScore']

	def recognize_amazon(self, audio_data, bucket_name=None, access_key_id=None, secret_access_key=None, region=None,
						 job_name=None, file_key=None):
		"""
		...
		if check_existing:
			...
			try:
				...
				if exc.response['Error'][
					'Code'] == 'BadRequestException' and "The requested job couldn't be found" in str(exc):
